...
use App\Model\Franchiseimages;
use DB;
class Franchisecompanydetail extends Model
{
    //
     protected $table = 'franchise_company_details';
     
    public function adddetails($request,$id,$existing_data){
        //         print_r($request->file());
        //         die();
        $deleterecord = Franchisecompanydetail::where('user_id', $id)->delete();
          if($request->file()){
             if($request->file('mul_imgs')){
                ...
                for ($i = 0; $i < count($request->file('mul_imgs')); $i++) {
                    ...
                    $objFundaraiserImage = new Franchiseimages();
         ...
         $objFundraisercompanydetail = new Franchisecompanydetail();
         ...
         $objFundraisercompanydetail->min_sft =$request->input('min_sft');
         $objFundraisercompanydetail->max_sft =$request->input('max_sft');
     ...
     public function franchisecompanydetails($id){
        $result = DB::table('users')
                ...
                ->GroupBy('franchise_images.user_id')
                ->where("users.id",$id)
                ->get();
