var Fundraiser = function () {
    ...
    var fundraiserdashborad = function () {
...
function verfy_details()
{
    $('body').on('click', '.verificationprofile', function (evt) {

        var deleteprofileid = $(this).attr("data-id");
        evt.preventDefault();
        setTimeout(function () {
            $('#btnverficationModal').click();
        }, 500);
    });
    var verficationform = $('#frmVerfication');
    var verifcationrules = {
        ver_address: {
            required: true,
            maxlength: 250,
        },
    ...
    handleFormValidateWithMsg(verficationform, verifcationrules, verifcationmessages,
            function (verficationform)
            {
                verficationform.submit();
            }
    );

}
function country_change(country_val, sttae_class)
{
    var val = country_val;
    $.ajax({
        type: "POST",
        ...
        success: function (data) {
            ...
            $('.' + sttae_class).html(html);
        }
    });

}
function state_change(state_val, city_class)
{
    var val = state_val;
    $.ajax({
        type: "POST",
        ...
        success: function (data) {
            var output = JSON.parse(data);
            if (output.length > 0) {
                $('.' + city_class).attr('name', city_class);
            } else {
                $('.' + city_class).removeAttr("name");
            }
            var temp_html = '';
            ...
            $('.' + city_class).html(html);
        }
    });
}
function country_state_change()
{
    $('body').on("change", "#country", function () {
        country_change($(this).val(), 'state');
    });
    $('body').on("change", "#state", function () {
        state_change($(this).val(), 'city');
    });

    $('body').on("change", "#ver_country", function () {
        country_change($(this).val(), 'ver_state');
    });
    $('body').on("change", "#ver_state", function () {
        state_change($(this).val(), 'ver_city');
    });
}
