...
function getstatushistory(id)
{
  ...
  $.ajax({
      type: 'GET',
      url: baseurl+"get-status-history/"+id,
      dataType:'json',

      success: function (result) {
        prepare_aprrove_history(result);
          hideprogress()
      }
  });
}
function prepare_aprrove_history(result)
{

    var html="";
    html+="<table class='table table-hover table-checkable order-column full-width dataTable no-footer'>";
    html+="<thead>";
    html+="<tr>";
    html+="<th>S.R.No</th>";
    html+="<th>Staff Member Name</th>";
    html+="<th>Status Updated Date</th>";
    html+="<th>Updated Status</th>";
    html+="<th>Notes</th>";
    ...
    for(var i=0;i<result.length;i++)
    {
        ...
        html+="<td>" + result[i]['created_at']  +"</td>";
        html+="<td>" + result[i]['status_to']  +"</td>";
...
function showprogress() {
 ...
 $(document).ready(function(){
    $(".inrformat").each(function(){
        change_to_inr_format(this,1);
      });
      $(".inrformat") .on('change',function()
      {
          change_to_inr_format(this,0)
      });
 });
 function change_to_inr_format(thisVar,isReadyTime)
 {
    ...
    if (isReadyTime==0)
    {
        $('input[name="'+originalTextboxId+'"]').valid();
    }
